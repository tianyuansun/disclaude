...
export class TaskQueue<T extends Task = Task> {
  ...
  protected pendingQueue: string[] = []; // Ordered by priority
  protected completedTasks: T[] = [];
   ...
   */
  enqueue(options: BaseTaskOptions, createTask?: (options: BaseTaskOptions) => T): T {
    const task = createTask
      ? createTask(options)
      : this.createDefaultTask(options);

    this.tasks.set(task.id, task as T);
   ...
   */
  protected createDefaultTask(options: BaseTaskOptions): Task {
    return {
      ...options,
      status: 'pending',
      createdAt: new Date(),
   ...
   */
  enqueueBatch(optionsList: BaseTaskOptions[], createTask?: (options: BaseTaskOptions) => T): T[] {
   ...
   */
  protected areDependenciesSatisfied(task: Task): boolean {
